...
func TestSeeking(t *testing.T) {
	...
	// Test trimming
	slim := minlz.RemoveIndexHeaders(index)
	if slim == nil {
		t.Error("Removing headers failed")
	}
	restored := minlz.RestoreIndexHeaders(slim)
	...
	for _, skip := range testSizes {
		t.Run(fmt.Sprintf("noSeekSkip=%d", skip), func(t *testing.T) {
			dec := minlz.NewReader(io.NopCloser(bytes.NewReader(compressed.Bytes())))
			_, err := dec.ReadSeeker(restored)
			if err == nil {
				t.Fatal("expected seeker to fail")
			}
		})
		t.Run(fmt.Sprintf("seekSkip=%d", skip), func(t *testing.T) {
			dec := minlz.NewReader(io.ReadSeeker(bytes.NewReader(compressed.Bytes())))
			seeker, err := dec.ReadSeeker(restored)
			if err != nil {
				t.Fatal(err)
			}
			buf := make([]byte, 25)
		...
		t.Run(fmt.Sprintf("seekIndexSkip=%d", skip), func(t *testing.T) {
			...
			seeker, err := dec.ReadSeeker(index)
			if err != nil {
				t.Fatal(err)
			}
			buf := make([]byte, 25)
...
func TestSeekingStreamIndex(t *testing.T) {
	compressed := bytes.Buffer{}
